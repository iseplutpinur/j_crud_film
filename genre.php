    ...
    <div class="card shadow mt-3">
      <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
          <label class="h6">Data Genre</label>
          <a href="./genre-form.php" class="btn btn-sm btn-info">Tambah</a>
        </div>
      </div>
      <div class="card-body">
        <table class="table table-responsive-md table-striped table-hover">
          ...
          <tbody>
            <?php
            $query = "SELECT pt.*, (SELECT count(*) FROM film as bu WHERE pt.id = bu.genre_id ) as film FROM genre as pt";
            ...
            while ($row = mysqli_fetch_assoc($result)) {
              ...
              $btn_edit =  '<a href="./genre-form.php?edit=' . $row->id . '" class="btn btn-sm btn-primary">Edit</a>';
              $btn_delete =  '<a href="./genre.php?delete=' . $row->id . '" onclick="return confirm(\'Apakah anda yakin?\')" class="btn btn-sm btn-danger">Hapus</a>';
              echo "<tr>
              <th>$counter</th>
              <td>{$row->nama}</td>
              <td>{$row->film}</td>
